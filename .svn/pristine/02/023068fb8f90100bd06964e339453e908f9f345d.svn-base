  ...
  /*  1-1  ReadOnly 모달 / 삭제  수정 가능  =>  erp에서 연결하지 않는 이상은 수정 삭제 가능  :  모달 실행 */
   ...
   /*  2 .단건조회 등등  모달창 값 초기화  */
	...
	function oemOneResult(data) {
			...
			// 그룹코드 폼 데이터 설정
			oemInitForm(data.oempart);
			
			
			// 숫자 -> 한글로 , 데이터값 바로 박음 
			fn_change_hangul_money(data.oempart.supply_val,data.oempart.estimate_cnt);
	  ...
	  */
		...
		function oemDetailList(currentPage) {  
			...
			var resultCallback = function(data,currentPage) {
				...
				//목록 조회 결과 
				oemListDetailResult(data,currentPage);
				console.log(" 목록뿌려주기 조회결과 data ",data);
			};
			
			/*  보낼 링크 / 컨트롤러로 보낼 방식 /  받을 방식 ,데이터,, 비동기? 동기,     돌려 줄 함수  */
			callAjax("/business/oemManagementList.do", "post", "text",  true,param, resultCallback); //text
